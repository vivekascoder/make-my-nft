...
class FA2_core(sp.Contract):
    ...
    def update_token_metadata(self, params):
        """
        This entry point to modify the token metadata only by the admins.
        """
        sp.set_type(params, sp.TList(
            sp.TRecord(
                token_id = sp.TNat,
        ...
        # Check if the user is admin or crowdsale otherwise panic.
        sp.verify(
            (self.is_administrator(sp.sender) | self.is_whitelisted(sp.sender)),
            message = self.error_message.not_admin()
        )

        # Loop over the list and update the metadata of the tokens.
        sp.for data in params:
            self.data.token_metadata[data.token_id] = sp.record(
                token_id    = data.token_id,
                token_info  = data.metadata
            )
...
class FA2_administrator(FA2_core):
    ...
    def is_whitelisted(self, sender):
        return self.data.whitelisted_addresses.contains(sender)
...
class FA2_mint(FA2_core):
    # @sp.entry_point
    # def set_crowdsale(self, params):
    #     sp.verify(
    #         self.is_administrator(sp.sender), self.error_message.not_admin()
    #     )
    #     self.data.crowdsale = params
    
    @sp.entry_point
    def whitelist_address(self, address):
        sp.set_type(address, sp.TAddress)
        sp.verify(
            self.is_administrator(sp.sender), self.error_message.not_admin()
        )
        self.data.whitelisted_addresses.add(address)
    ...
    def blacklist_address(self, address):
        ...
        sp.verify(
            self.data.whitelisted_addresses.contains(address),
            "ADDRESS_IS_NOT_WHITELISTED"
        )
        self.data.whitelisted_addresses.remove(address)
    ...
    def mint(self, params):
        sp.verify(
            # (self.is_administrator(sp.sender) | (sp.sender == self.data.crowdsale)),
            (self.is_administrator(sp.sender) | self.is_whitelisted(sp.sender)),
            message = self.error_message.not_admin()
        )
...
class FA2(FA2_change_metadata, FA2_token_metadata, FA2_mint, FA2_administrator, FA2_pause, FA2_core):
    ...
    def __init__(self, config, metadata, admin):
        ...
        FA2_core.__init__(
            ...
            paused = False, 
            administrator = admin, 
            whitelisted_addresses = sp.set(l=[], t=sp.TAddress)
        )
